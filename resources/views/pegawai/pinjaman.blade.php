...
<body ">
                    ...
                    <li class="menu-item-has-children dropdown show open">
                        ...
                        <ul class="sub-menu children dropdown-menu  show">
                           <li ><i class="  fa fa-circle "></i><a  href="datapegawai">Data Pegawai</a></li>
                            ...
                            <li><i class=" active fa fa-circle"></i><a class="active"  href="pinjaman">Pinjaman</a></li>
                        </ul>
                    </li>
                    <li class="menu-item-has-children dropdown">
        ...
        <div class="content pb-0">
            ...
            <!-- Widgets  -->
             <div class="row" style="margin-bottom: 30px">
                <div class="col-lg-3 col-md-6">
                    <h3 style="color:#8c8e8d;"><b>Pinjaman</b></h3>
                </div>
                                ...
                                <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                   <thead>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{$pinjaman->kode}}</td>
                                            <td>{{$pinjaman->namapegawai}}</td>
                                            <td>{{$pinjaman->ttl_peminjaman}}</td>
                                            <td>{{$pinjaman->tanggal}}</td>
                                            <td><button type='button' class='editdata' data-toggle='modal' data-target='#largeModal' data-info="{{$pinjaman->kode}},{{$pinjaman->namapegawai}},{{$pinjaman->ttl_peminjaman}},{{$pinjaman->tanggal}}">Edit</button><button type='button' class='deletedata' data-toggle='modal' data-target='#deletedata' data-info="{{$pinjaman->kode}},{{$pinjaman->namapegawai}},{{$pinjaman->ttl_peminjaman}},{{$pinjaman->tanggal}}">Hapus</button></td>
                                        </tr>
                                        @endforeach
                                    </tbody> 
            ...
            <div class="modal fade" id="mediumModal" tabindex="-1" role="dialog" aria-labelledby="mediumModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <form id="tambah" method="POST" action="{{ url('peminjamanAdd') }}">
                        {{ csrf_field() }}
                        <div class="modal-header">
                            <a class="modal-title" id="mediumModalLabel">Tambah Pinjaman</a>
                            ...
                            <div class="row form-space">
                                ...
                                <div class="col-lg-5 text-left">
                                    <select data-placeholder="" type="text" id="" name="namapegawai" class=" form-control form-input standardSelect required" tabindex="1"> 
                                        <option  class="option" value=""  selected>Nama Pegawai</option>
            ...
            <div class="modal fade" id="largeModal" tabindex="-1" role="dialog" aria-labelledby="largeModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" >
                    <div class="modal-content">
                    <form id="editdata" method="POST" action="{{ url('peminjamanUpdate') }}">
                        {{ csrf_field() }}
                        <div class="modal-header">
                            <a class="modal-title" id="largeModalLabel">Ubah Pinjaman</a>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <div class="row form-space">
                                ...
                                <div class="col-lg-5 text-left">
                                    <input  type="text" id="ubahkodepinjaman" name="ubahkodepinjaman" placeholder="" class="required form-control form-input" readonly>
                                </div>
                            </div>
                            <div class="row form-space">
                                <div class="col-lg-3 text-left">
                                ...
                                <div class="col-lg-5 text-left">
                                    <select data-placeholder="" type="text" id="ubahnamapegawai" name="ubahnamabarang" class=" form-control form-input standardSelect required" tabindex="1"> 
            ...
            <div class="modal fade" id="largeModal1" tabindex="-1" role="dialog" aria-labelledby="largeModalLabel" aria-hidden="true">
             ...
             <div class="modal fade" id="deletedata" tabindex="-1" role="dialog" aria-labelledby="largeModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" >
                    <div class="modal-content">
                    <form id="" method="POST" action="{{ url('peminjamanDelete') }}">
                        {{csrf_field()}}
                        <div class="modal-header">
                            <a class="modal-title" id="largeModalLabel">Hapus Data Pinjaman</a>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
                          ...
                          var table = $('#bootstrap-data-table').DataTable({
                            ...
                            lengthChange: false,
                             buttons: [
                                { 
                                    extend: 'excel',
                                    title: 'Estu SAE - Retur Penjualan',
                                    exportOptions: {
                                        columns: [ 0, 1, 2, 3, 4 ]
                                    }
                                }
                        ...
                        var column = this;
                        var select = $('<select class="form-control btn btn-primary placeholder  "><option value="">Pegawai</option><option value="">Semua</option></select>')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            rules: {ubahnamapegawai: { required: true },
                    ubahnamabarang: { required: true },
                    ubahjumlah: { required: true },
                    nomornota: { required: true },
                    namatoko: { required: true},
                    ubahtanggalmasuk: { required: true }
                    },
            messages: {
                    ...
                    nomornota: { required: "Nomor Nota harus diisi" },
                    namatoko: { required: "Nama Toko harus diisi" },
                    ubahtanggalmasuk: { required: "Tanggal harus diisi" }
                    },
            errorPlacement: function(error, element) {
            if (element.is('select:hidden')) {
                error.insertAfter(element.next('.nice-select'));
     ...
     <script type="text/javascript">
        ...
        function fillmodalData(details){
            ...
            $('#ubahkodepinjaman').val(details[0]);
            $('#ubahnamapegawai').find(':selected').val(details[0]).text(details[1]);
            $('#ubahjumlah').val(details[2]);
            $('#ubahtanggal').val(details[3]);
            
            var date = new Date(details[4]);
           
            $('#ubahtanggalmasuk').val( date.getFullYear() + '-' + ("0" + (date.getMonth() + 1)).slice(-2) + '-' + ("0" +date.getDate()).slice(-2) );
